(function ($) {
	...
	$.fn.shareButtons = function (url, options) {
		...
		var shareURLs = {
			...
			'linkedin': 'https://www.linkedin.com/shareArticle?mini=true&url=' + url_encoded
		};
		...
		if(options.linkedin) {
			shareURLs['linkedin'] += '&title=' + encodeURIComponent(options.linkedin.title);
		}
		...
		this.each(function(i){
			...
			if(options.linkedin){
				socialButtonsPopUp.append($('<a class="fa fa-linkedin" href="' + shareURLs.linkedin + '" ></a></a>'));
			}
		...
		$('.socialPlugin a').on('click', function(e) {
			...
			window.open(url, 'Learn PHP Today', 'toolbar=no, location=no, directories=no, status=no,' +
